...
public class LeetCode2 {
	...
	/*1.二进制中1的个数， 思路不断除以2，mod 2
	 * 注意，这仍然不是最快的方法
	 * 对于32位整型有计数方法，64位也有，这个指令仅限于C++
	...
	public int NumberOf1(int n) {
		...
		int count = 0;
		while(n>0) {
			count += n%2;//这里不需要自己做优化，编译器用的就是位移运算
			n /= 2;//请勿自作聪明
		}
		return count;*/
	 ...
	 * 每一行都按照从左到右递增的顺序排序，
	 * 每一列都按照从上到下递增的顺序排序
	 * 思想：折半查找，先找到所在行，再找所在列
	 * 思路：从左上角开始，若小则删除一行，若大，则删除一列
	 * 要求，这个数组行列相同
	 * */
    ...
    /*13.给定一个double类型的浮点数base和int类型的整数exponent。
     * 求base的exponent次方。
     * 快速幂算法*/
    ...
    /*  30.连续数组最大和
     * 	思想：分治？动态规划
     * 	思想是动态规划，如果加上当前位置的数后，和比当前位置的数小，则重新开始
     * */
    ...
    /* 32.一个整型数组里，除两个数字外，其它数字都出现了偶数次
     * 	使用xor，异或的性质 a^a = 0; 0^a = a;
     * */
    ...
    */
    ...
    /* 45.给定一个数组和滑动窗口的大小，找出所有滑动窗口里数值的最大值。
     * 即如果有k个滑动窗口，则有k个最大值	·
     * */
    ...
    boolean isSame(TreeNode aroot,TreeNode broot){
        ...
        if(aroot!=null && broot!=null){
            if(aroot.val!=broot.val)
                return false;
            return isSame(aroot.left,broot.left) &&
                isSame(aroot.right,broot.right);
        }
        return false;
    }
    /*方法2，直接判断自己和自己是否是镜像的*/
    boolean isSymmetrical(TreeNode pRoot,TreeNode qRoot){
        ...
        if(pRoot!=null && qRoot!=null && pRoot.val==qRoot.val)
            return isSymmetrical(pRoot.right,qRoot.left) &&
            isSymmetrical(pRoot.left,qRoot.right);
        return false;
    }
    ...
    /* 52.判断链表中是否有环，并返回环的第一个节点
     * 用两个指针，一个快指针，一个慢指针，当两个相遇时
     * */
    ...
    /* 57.输入一个正整数数组，把数组里所有数字拼接起来排成一个数，
     * 打印能拼接出的所有数字中最小的一个。
     * 例如输入数组{3，32，321}，
    ...
    /* 58.把只包含质因子2、3和5的数称作丑数（Ugly Number）。
     * 例如6、8都是丑数，
    ...
    public int GetUglyNumber_Solution(int index) {
        ...
        while(cnt<index-1){
            ...
            if(tmp==2*result[p2]) p2++;
            if(tmp==3*result[p3]) p3++;
            if(tmp==5*result[p5]) p5++;
            result[++cnt] = tmp;
        }
    ...
    /* 60.求出1~13的整数中1出现的次数,并算出100~1300的整数中1出现的次数？
     * 方法：计算每一位中1出现的次数
     * */
    ...
    /* 61.判断字符串是否表示数值(包括整数和小数)
     * +100、5e2、-123、3.14、-1E-16	正例
	...
	/* 65.输入某二叉树的前序遍历和中序遍历的结果，请重建出该二叉树。
	 * 假设输入的前序遍历和中序遍历的结果中都不含重复的数字.
	 * 思考：如何重建
	 * 首先在先序遍历中找到根节点，然后在中序遍历序列中找左右子树，然后找左右子树的根节点，
	 * 递归
	 * */
